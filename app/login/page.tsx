      ...
      <div style={{ width: '100%', maxWidth: 340 }}>
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: '#6B7280', letterSpacing: 2, textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Nome</label>
              <input type="text" value={nome} onChange={e => setNome(e.target.value.replace(/[^a-zA-ZÀ-ú]/g, ''))} placeholder="Mario" maxLength={20} autoFocus
                style={{ width: '100%', background: '#0C111D', border: '1.5px solid #1F2937', borderRadius: 14, padding: '14px 14px', color: '#F9FAFB', fontSize: 16, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>
            <div>
              <label style={{ fontSize: 10, fontWeight: 700, color: '#6B7280', letterSpacing: 2, textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Cognome</label>
              <input type="text" value={cognome} onChange={e => setCognome(e.target.value.replace(/[^a-zA-ZÀ-ú]/g, ''))} placeholder="Rossi" maxLength={20}
                style={{ width: '100%', background: '#0C111D', border: '1.5px solid #1F2937', borderRadius: 14, padding: '14px 14px', color: '#F9FAFB', fontSize: 16, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>
          </div>

          <p style={{ fontSize: 11, color: '#374151', margin: 0 }}>Nessuna password richiesta</p>
